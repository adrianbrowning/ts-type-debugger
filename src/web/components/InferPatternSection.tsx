  ...
  const renderContent = () => {
    switch (stepType) {
      case 'infer_pattern_start':
        ...
        return <InfoRow label="Pattern" value={pattern} monospace />;
      ...
      case 'infer_pattern_match':
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.sm }}>
            <InfoRow label="Check" value={expression} monospace />
            <div>
      ...
      case 'infer_binding':
        ...
        const bindingMatch = expression.match(/^(\w+)\s*=\s*(.+)$/);
        if (bindingMatch) {
          return (
            <div style={{ display: 'flex', flexDirection: 'column', gap: theme.spacing.sm }}>
              <InfoRow label="Variable" value={bindingMatch[1]} monospace />
              <InfoRow label="Value" value={result || bindingMatch[2]} monospace />
            </div>
          );
        }
        return <InfoRow label="Binding" value={expression} monospace />;

      case 'infer_pattern_result':
        return <InfoRow label="Result" value={result || expression} monospace />;

      default:
        return <InfoRow label="Expression" value={expression} monospace />;
    }
  };
